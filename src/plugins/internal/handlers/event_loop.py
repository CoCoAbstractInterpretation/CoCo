...
def cs_port_postMessage(G, event):
    ...
    args = [NodeHandleResult(obj_nodes=copied_messages)]
    with G.eventRegisteredFuncs_lock:
...
def cs_chrome_runtime_sendMessage(G, event):
    ...
    # print("why torture me")
    # G.debug_sink_in_graph(sender_responseCallback)
...
def bg_chrome_tabs_sendMessage(G, event):
    ...
    sendResponse = G.get_objs_by_name('sendResponse', scope=G.cs_scopes[0], branches=[])
...
from src.plugins.internal.modeled_js_builtins import handle_eval
    # event = {'eventName': eventName, 'info': info, 'extra':extra, 'caller_ast':caller_ast}

def handle_eval_in_event_loop(G, event):
    caller_ast = event['caller_ast']
    extra = event["extra"]
    messages = G.get_prop_obj_nodes(event['info'], prop_name = 'message')
    strings = NodeHandleResult(obj_nodes=messages)
    handle_eval(G, caller_ast, extra, None, strings)
...
event_listener_dic = {
    ...
    "chrome_tabs_executeScript_event":["cs_chrome_tabs_executeScript_event", handle_eval_in_event_loop]
}
